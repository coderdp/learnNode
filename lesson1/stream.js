...
function ReadStream(){
    // 打开一个流:
var rs=fs.createReadStream('./file/test.txt','utf-8');
...
function WriteStream(){
    //Stream写文件
    var ws1=fs.createWriteStream('./file/WriteStream.txt','utf-8');
...
function Copy()
{
    ...
    var rs=fs.createReadStream('./file/WriteStream.txt','utf-8');
    var ws=fs.createWriteStream('./file/NewCopy.txt','utf-8');
    rs.on("end",function(){
        console.log('rs,end');
    })
    rs.pipe(ws);
